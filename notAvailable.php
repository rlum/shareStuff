...
<body>
<h1> UNAVAILABLE STUFF </h1>

<FORM><INPUT TYPE="button" VALUE="Back" onClick="history.go(-1);return true;"><br>
	<br>
	<br><span class="auto-style1">Click on an item to add it to the wait queue</span></FORM>
...
if ($db_conn=OCILogon("ora_i0r7", "a41671785", "ug")){
	...
	minus 
	select i.i_number, i.value, i.description, i.available from item i , inqueue q
...
<?php
	while($row = OCI_Fetch_Array($parsed, OCI_BOTH)){
		echo "<TR>";
		echo "<TD>";
		echo "<a href=\"enqueue.php?item=$row[0]&myperson=$mylocalvar\">";
		echo $row[0];
		echo "</a>";
		echo "</TD><TD>";
		echo $row[1];
